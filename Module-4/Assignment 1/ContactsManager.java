...
class ContactsManager {
    ...
    ContactsManager(){
        int fiveHunderd = 500;
        this.myFriends = new Contact[fiveHunderd];
        this.friendsCount=0;
    }
    ...
    public Contact searchContactByEmail(String email) {
        //  Your code goes here....
        for(int i=0;i<friendsCount;i++){
            // System.out.println("before ---->"+ myFriends[i].getPhoneNumber() +"------" + myFriends[i].getEmail());
            if(myFriends[i].getEmail().equals(email)){
                //  System.out.println("After---->"+ myFriends[i].getPhoneNumber());
                 return myFriends[i];
            }    
        }    
        return null;
